...
<body>
	...
	<div class="container">
		<form action="selectLikeVideo">
			<input class="btn btn-info dropdown-toggle" type="text" name="title"
				placeholder="请输入你要查询的标题">
			<td><label for="speaker_id">请选择老师</label></td>
			<td colspan="4" class="btn btn-info dropdown-toggle"><select
			...
			<td><label for="course_id">请选择课程</label></td>
			<td colspan="4" class="btn btn-info dropdown-toggle"><select
			...
			</select></td> <input class="btn btn-info dropdown-toggle" type="submit" value="搜索">
		</form>
	</div>
			...
			<tbody>
				<c:if test="${! empty selectLikeVideo }">
					<c:forEach items="${selectLikeVideo }" var="selectLikeVideo">
						...
						<tr>
							...
							<td
								style="overflow: hidden; white-space: nowrap; text-overflow: ellipsis;">${selectLikeVideo.detail }</td>
							...
							<td><a
								onclick="delCourseById('${selectLikeVideo.video_id }','${selectLikeVideo.title }')">X</a></td>
						</tr>
					</c:forEach>
				</c:if>
				...
				<c:if test="${ empty selectLikeVideo }">
					<c:forEach items="${videoList }" var="videoList">
						...
						<tr>
							<td><input type="checkbox"  name="c" value="${videoList.video_id }"></td>
							<td>${videoList.video_id }</td>
							<td>${videoList.title }</td>
							<td
								style="overflow: hidden; white-space: nowrap; text-overflow: ellipsis;">${videoList.detail }</td>
							...
							<td><a
								onclick="delCourseById('${videoList.video_id }','${videoList.title }')">X</a></td>
						</tr>
					</c:forEach>
				</c:if>
			</tbody>
